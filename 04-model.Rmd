...
winprob = matrix %>% 
  mutate(
    ...
    pregameodds2 = predict(model.pregameodds2, newdata = ., type = "response"),
    pregameodds3 = predict(model.pregameodds3, newdata = ., type = "response")
  ) %>% 
  select(season, round, tie, t1win, made_minute, away, t1goaldiff, t1awaygoaldiff,
         neutral, pregameodds, pregameodds2, pregameodds3)
```

```{r}
errors = winprob %>% 
  mutate(
    errorN = t1win - neutral, errorO = t1win - pregameodds, errorO2 = t1win - pregameodds2, errorO3 = t1win - pregameodds3,
    sqrErrorN = errorN^2, sqrErrorO = errorO^2, sqrErrorO2 = errorO2^2, sqrErrorO3 = errorO3^2
  ) %>% 
...
errors %>% 
  select(-count) %>% 
  gather(key = method, value = rtMeanSqrError, neutral, pregameodds, pregameodds2, pregameodds3) %>% 
  ggplot() +
