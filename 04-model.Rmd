...
```{r}
matrix2 = matrix %>% 
  mutate(neutral = predict(model.neutral, newdata = ., type = "response"))
```


```{r}
model.blended = locfit(
  t1win ~ made_minute + t1goaldiff + t1awaygoaldiff + 
    pcthomeg1 + pcthomeg2 + neutral,
  data = matrix2,
  family = 'binomial'
)
```
...
winprob = matrix %>% 
  mutate(
    ...
    blended = predict(model.blended, newdata = matrix2, type = "response")
  ) %>% 
  select(season, round, tie, t1win, made_minute, away, t1goaldiff, t1awaygoaldiff, neutral, pregameodds, blended)
```

```{r}
errors = winprob %>% 
  mutate(
    errorN = t1win - neutral, errorO = t1win - pregameodds, errorB = t1win - blended,
    sqrErrorN = errorN^2, sqrErrorO = errorO^2, sqrErrorB = errorB^2
  ) %>% 
  ...
  summarise(
    ...
    blended = sqrt(mean(sqrErrorB))
  )

errors %>% 
  select(-count) %>% 
  gather(key = method, value = rtMeanSqrError, neutral, pregameodds, blended) %>% 
  ggplot() +
