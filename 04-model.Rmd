...
winprob = matrix %>% 
  ...
  select(season, round, tie, t1win, made_minute, away, t1goaldiff, t1awaygoaldiff, neutral, pregameodds, pregameodds2)
```

```{r}
errors = winprob %>% 
  mutate(
    errorN = t1win - neutral, errorO = t1win - pregameodds, errorO2 = t1win - pregameodds2,
...
errors %>% 
  select(-count) %>% 
  gather(key = method, value = rtMeanSqrError, neutral, pregameodds, pregameodds2) %>% 
  ggplot() +
