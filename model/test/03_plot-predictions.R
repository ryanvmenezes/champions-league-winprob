...
predictions = read_rds(here('model','v2', 'predictions.rds'))
...
plots %>% write_rds(here('model', 'v2', 'all-plots.rds'), compress = 'gz')
...
plots %>% 
  ...
  pwalk(
    function(comp, season, round) {
      plotsfolder = here('model', 'v2', 'plots')
  ...
  mutate(
    outpath = pmap_chr(
      ...
      function(comp, season, round, team1, team2) {
        roundfolder = here('model', 'v2', 'plots', comp, season, round)
