...
colnames(ODDS) <- c('X__1','X__2','X__3','X__4','X__5','X__6','X__7')
```
...
```{r}
odds %>% write_csv('raw/cleaned-odds.csv')
odds = read_csv('raw/cleaned-odds-updated.csv', col_types = c(.default = 'c'))
```
...
games.odds.xwalk = results %>% 
  ...
  left_join(odds %>% select(season,hometeamfull = hometeam,awayteamfull = awayteam,pcthome,pctdraw,pctaway),
            by = c('season','hometeamfull','awayteamfull'))
```
